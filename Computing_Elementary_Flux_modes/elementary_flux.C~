...
{
int i,j,m,n,p,row,col,set=0,set1=0,set2=0,n_row,l=0,x=0,q,r,s,t,*a,*b,*c,*a1,*b1,*c1,count_rev=0,count_rev_final,count_irrev=0,count_irrev_final,count=0,flag=0;
float k,k1,k3;
int *rev,*irrev,**stoimat2,**stoimat,**elementary_mode,**elementary_mode2;
char ch,ch1;
int line=0,line1=0,line2=0,space=0,**stoimat3,rows,columns;
char *s1;
FILE *fp,*fp1,*fp2;
if(argc!=4)
    {
      printf("\n\nYou did not give the input file(.txt) name\n\n Run command again and give the input file name correctly.\n\n");
      exit(1);
    }
  fp=fopen(argv[3],"rb+");
  if(fp==NULL)
  ...
  rows=line;
  columns=space+1;
  printf("\nStoichiometic Matrix:-the number of rows are %d and no of columns are %d \n\n\n",rows,columns);
  printf("\n\n\n");
  stoimat3=allocate2((rows*columns),(columns*rows));
  fp1=fopen(argv[1],"rb+");
  if(fp1==NULL)
  {
    printf("Metabolites File can not be opened\n");
    fclose(fp);
    exit(2);
  }
  ...
  printf("\nthe number of Metabolites are %d  \n\n\n",line1);
  printf("\n\n\n");

  fp2=fopen(argv[2],"rb+");
  if(fp2==NULL)
  {
    printf("reactions File can not be opened\n");
    fclose(fp);
    fclose(fp1);
    exit(2);
  }
  ...
  printf("\nthe number of reactions are %d  \n\n\n",line2);
  printf("\n\n\n");

  printf("The stoichiometric matrix read from input file is as follows:\n\n\n");
  rewind(fp);
  for(i=0;i<rows;i++)
    for(j=0;j<columns;j++)
      fscanf(fp,"%d",&stoimat3[i][j]);
    
    for(i=0;i<rows;i++)
    {
       for(j=0;j<columns;j++)
	 printf("	%d",stoimat3[i][j]);
       printf("\n");
    }
  ...
  stoimat=allocate2((columns*rows),(rows*columns));
  
  for(i=0;i<columns;i++)
    for(j=0;j<rows;j++)
      stoimat[i][j]=stoimat3[j][i];
 ...
 stoimat2=allocate2((col*row),(row*col));
 ...
 printf("Calculation\n\n");
  for(j=0;j<col;j++)
  {
    ...
    if(l==-1&&count_rev_final!=0)
    {
      for(p=0;p<count_rev_final;p++)
      {
	for(q=0;q<col;q++)
	  stoimat2[p][q]=stoimat[p][q];
	count_rev++;
	count++;
      }
      ...
      for(p=0;p<count_rev_final;p++)
	for(q=0;q<n_row;q++)
	  elementary_mode2[p][q]=elementary_mode[p][q];
